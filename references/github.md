...
先确定规范 owner/repo，检索本地已有报告，聚焦本次变化和未回答问题。优先使用 gh 的只读 API，再按问题阅读 README、相关代码、文档、近期 issue 或讨论。
...
多项目比较使用一致维度，区别未确认和不支持。按 archive.md 保存，不使用旧报告目录或其索引器。若由 X 帖子引出且项目研究在任务范围内，在两份报告之间建立文件链接，帖子分析仍需完成。
